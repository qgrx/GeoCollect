...
export default function AdminCards({ cardPool, cardTypes, onAddCard, onEditCard, onDeleteCard, setMsg, imgUpload }) {
  const { t } = useT();
    ...
    <div>
      ...
      {(selectedType||editCard)&&(
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:14,fontSize:12,color:"#888"}}>
          <button onClick={()=>{setSelectedType(null);setEditCard(null);}} style={{background:"none",border:"none",color:"#e74c3c",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif",padding:0}}>Tous les types</button>
          {selectedType&&<><span>›</span><span style={{color:"#fff",fontWeight:700}}>{selectedType} ({cardPool.filter(c=>c.type===selectedType).length})</span></>}
          {editCard&&<><span>›</span><span style={{color:"#f9ca24",fontWeight:700}}>✏️ {editCard.name}</span></>}
        </div>
      )}
      {!selectedType&&!editCard&&(
      ...
      {(selectedType||editCard)&&(
        <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:240}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:4}}>{editCard?(<><button onClick={async()=>{if(!editCard.name.trim()){setMsg("❌ Nom requis.");return;}const payload={...editCard, image_url: editCard.image, image_url_thumb: editCard.thumbnail}; if(payload.minPrice!==undefined){payload.min_price=payload.minPrice; delete payload.minPrice;} delete payload.image; delete payload.thumbnail; const err=await onEditCard(payload);if(err){setMsg("❌ "+err);return;}setEditCard(null);setMsg(`✅ "${editCard.name}" mis à jour !`);}} style={{flex:1,...BTN("linear-gradient(135deg,#e74c3c,#c0392b)"),padding:"11px",borderRadius:10}}>Enregistrer ✏️</button><button onClick={()=>setEditCard(null)} style={{...BTN("#ffffff18"),padding:"11px",borderRadius:10}}>Annuler</button><button onClick={async()=>{if(!window.confirm(`Supprimer définitivement "${editCard.name}" ?`)) return;const name=editCard.name; const type=editCard.type;const err=await onDeleteCard(editCard.id);if(err){setMsg("❌ "+err);return;}setEditCard(null);setSelectedType(type);setNc(prev=>({...prev,type}));setMsg(`✅ "${name}" supprimée.`);}} style={{...BTN("linear-gradient(135deg,#e74c3c,#c0392b)","#fff"),padding:"11px",borderRadius:10}} title="Supprimer cette carte">🗑️</button></>):(<button onClick={async()=>{if(!nc.name.trim()){setMsg("❌ Nom requis.");return;}const payload={name:nc.name.trim(), type:nc.type||selectedType||cardTypes[0]||"", rarity:nc.rarity, image_url:nc.image, image_url_thumb:nc.thumbnail, desc:nc.desc, sellable:nc.sellable, min_price:nc.minPrice||null}; const err=await onAddCard(payload);if(err){setMsg("❌ "+err);return;}setMsg(`✅ "${nc.name}" créée !`);setNc({name:"",type:selectedType||cardTypes[0]||"",rarity:"commun",image:null,thumbnail:null,desc:"",sellable:true,minPrice:""});}} style={{flex:1,...BTN("linear-gradient(135deg,#e74c3c,#c0392b)"),padding:"11px",borderRadius:10}}>{t("admin_create_card")}</button>)}</div>
          </div>
