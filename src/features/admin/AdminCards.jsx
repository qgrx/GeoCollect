    ...
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {!editCard && !newCardMode && (()=>{const hiddenCards=(cardPool||[]).filter(c=>c.hidden);return hiddenCards.length>0&&(
          <button onClick={async()=>{if(!window.confirm(`Publier ${hiddenCards.length} carte(s) cachée(s) ? Elles deviendront visibles, comptées et utilisables par les joueurs.`))return;const {data,error}=await apiReleaseHiddenCards();if(error){setMsg("❌ "+error);return;}hiddenCards.forEach(c=>onUpdateCardInPool?.({...c,hidden:false}));setMsg(`✅ ${data?.released??hiddenCards.length} carte(s) publiée(s) !`);}}
