            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              ...
              <tbody>
                {[
                  ...
                  ["Or / victoire (hors limite)", <><input type="number" min={0} max={1000} value={limEdit.quizConsolationGold??5} onChange={e=>setLimEdit({...limEdit,quizConsolationGold:Math.max(0,+e.target.value)})} style={{...INP,width:60}}/> <span style={{color:"#aaa"}}>Or +</span> <input type="number" min={0} max={100} value={limEdit.quizConsolationForge??1} onChange={e=>setLimEdit({...limEdit,quizConsolationForge:Math.max(0,+e.target.value)})} style={{...INP,width:55,marginLeft:4}}/> <span style={{color:"#aaa"}}>PF</span></>, "Zéro inflation — boost Forge. Le champ PF ne sert plus qu'au DÉPÔT (renoncer au dépôt, dépôt plein, remplacement) : les PF d'une victoire « pour la gloire » sont réglés à la ligne suivante, par rareté."],
                  ["🔨 PF « pour la gloire » par rareté", (()=>{const D={commun:1,rare:2,épique:3,légendaire:5};const t=limEdit.quizConsolationForgeByRarity||D;const set=(k,v)=>setLimEdit(p=>({...p,quizConsolationForgeByRarity:{...D,...(p.quizConsolationForgeByRarity||{}),[k]:Math.max(0,+v)}}));return <div style={{display:"flex",flexWrap:"wrap",alignItems:"center",gap:4}}>{[["commun","Commun"],["rare","Rare"],["épique","Épique"],["légendaire","Lég."]].map(([k,lbl])=>(<span key={k} style={{display:"inline-flex",alignItems:"center",gap:3,marginRight:6}}><span style={{color:"#aaa"}}>{lbl}</span><input type="number" min={0} max={999} value={t[k]??D[k]} onChange={e=>set(k,e.target.value)} style={{...INP,width:52}}/></span>))}<span style={{color:"#aaa"}}>PF</span></div>})(), "Points de forge accordés pour une victoire « pour la gloire » (limite de geocoins atteinte, ou plafond hebdo de la rareté atteint), selon la rareté du geocoin disputé. Le plafond « Points de forge max / jour » ci-dessous s'applique ensuite. Sans effet sur le dépôt ni sur le mécénat, qui ont leurs propres réglages."],
                  ...
                  ["📅 Plafonds hebdo par rareté", <><span style={{color:"#aaa"}}>Rare</span> <input type="number" min={0} value={limEdit.quizWeeklyCapRare??100} onChange={e=>setLimEdit({...limEdit,quizWeeklyCapRare:Math.max(0,+e.target.value)})} style={{...INP,width:55}}/> <span style={{color:"#aaa",marginLeft:6}}>Épique</span> <input type="number" min={0} value={limEdit.quizWeeklyCapEpique??40} onChange={e=>setLimEdit({...limEdit,quizWeeklyCapEpique:Math.max(0,+e.target.value)})} style={{...INP,width:55,marginLeft:4}}/> <span style={{color:"#aaa",marginLeft:6}}>Lég.</span> <input type="number" min={0} value={limEdit.quizWeeklyCapLegendaire??1} onChange={e=>setLimEdit({...limEdit,quizWeeklyCapLegendaire:Math.max(0,+e.target.value)})} style={{...INP,width:55,marginLeft:4}}/> <span style={{color:"#aaa"}}>/ sem.</span></>, "Plafond d'acquisition HEBDOMADAIRE par rareté (0 = ∞). Au-delà : don (mécénat) ou gloire. Le shiny est toujours gagné mais compte dans le plafond."],
                  ["🛒 Marché — nouveaux geocoins / semaine", <><input type="number" min={0} value={limEdit.marketWeeklyNewCap??2} onChange={e=>setLimEdit({...limEdit,marketWeeklyNewCap:Math.max(0,+e.target.value)})} style={{...INP,width:60}}/> <span style={{color:"#aaa"}}>/ sem. (0 = ∞)</span></>, "Nombre max de geocoins ENCORE NON POSSÉDÉS qu'un joueur peut acheter au marché par semaine (0 = ∞). Les doublons (geocoins déjà possédés) restent illimités. Ne concerne ni le marché hors saison, ni le trésor quotidien, ni les dons de mécénat. Remise à zéro le lundi, avec les plafonds hebdo par rareté."],
                  ["🎁 Mécénat — dons / semaine", <><span style={{color:"#aaa"}}>Rare</span> <input type="number" min={0} value={limEdit.patronageWeeklyCapRare??10} onChange={e=>setLimEdit({...limEdit,patronageWeeklyCapRare:Math.max(0,+e.target.value)})} style={{...INP,width:55}}/> <span style={{color:"#aaa",marginLeft:6}}>Épique</span> <input type="number" min={0} value={limEdit.patronageWeeklyCapEpique??4} onChange={e=>setLimEdit({...limEdit,patronageWeeklyCapEpique:Math.max(0,+e.target.value)})} style={{...INP,width:55,marginLeft:4}}/> <span style={{color:"#aaa",marginLeft:6}}>Lég.</span> <input type="number" min={0} value={limEdit.patronageWeeklyCapLegendaire??1} onChange={e=>setLimEdit({...limEdit,patronageWeeklyCapLegendaire:Math.max(0,+e.target.value)})} style={{...INP,width:55,marginLeft:4}}/> <span style={{color:"#aaa"}}>/ sem.</span></>, "Nombre max de dons de mécénat par rareté et par semaine. Au-delà : le mécène joue pour la gloire."],
                  ["🎁 Mécénat — récompense (PF)", <><span style={{color:"#aaa"}}>Rare</span> <input type="number" min={0} value={limEdit.patronageRewardPfRare??1} onChange={e=>setLimEdit({...limEdit,patronageRewardPfRare:Math.max(0,+e.target.value)})} style={{...INP,width:55}}/> <span style={{color:"#aaa",marginLeft:6}}>Épique</span> <input type="number" min={0} value={limEdit.patronageRewardPfEpique??5} onChange={e=>setLimEdit({...limEdit,patronageRewardPfEpique:Math.max(0,+e.target.value)})} style={{...INP,width:55,marginLeft:4}}/> <span style={{color:"#aaa",marginLeft:6}}>Lég.</span> <input type="number" min={0} value={limEdit.patronageRewardPfLegendaire??100} onChange={e=>setLimEdit({...limEdit,patronageRewardPfLegendaire:Math.max(0,+e.target.value)})} style={{...INP,width:60,marginLeft:4}}/> <span style={{color:"#aaa"}}>PF</span></>, "Points de forge gagnés par le mécène selon la rareté offerte (hors plafond PF quotidien) + une gloire + le compteur « Le mécène »."],
                  ["🎁 Mécénat — critères", <><span style={{color:"#aaa"}}>Nouveau &lt;</span> <input type="number" min={0} value={limEdit.patronageNewDays??30} onChange={e=>setLimEdit({...limEdit,patronageNewDays:Math.max(0,+e.target.value)})} style={{...INP,width:48}}/> <span style={{color:"#aaa"}}>j · Ancien &gt;</span> <input type="number" min={0} value={limEdit.patronageOldDays??30} onChange={e=>setLimEdit({...limEdit,patronageOldDays:Math.max(0,+e.target.value)})} style={{...INP,width:48}}/> <span style={{color:"#aaa"}}>j · Rapide &lt;</span> <input type="number" min={0} value={limEdit.patronageFastSeconds??5} onChange={e=>setLimEdit({...limEdit,patronageFastSeconds:Math.max(0,+e.target.value)})} style={{...INP,width:44}}/> <span style={{color:"#aaa"}}>s · Collec.</span> <input type="number" min={0} value={limEdit.patronageCollectionThreshold??80} onChange={e=>setLimEdit({...limEdit,patronageCollectionThreshold:Math.max(0,+e.target.value)})} style={{...INP,width:48}}/></>, "Seuils de sélection du bénéficiaire : compte « nouveau » (< N jours) / « ancien » (> N jours), « rapide de la gâchette » (moyenne des 10 dernières réponses < N s), et seuil petite/grande collection."],
                  ["🎁 Mécénat — éligibilité du bénéficiaire", <><span style={{color:"#aaa"}}>Élargir jusqu'à &lt;</span> <input type="number" min={1} value={limEdit.patronageSeenDays??14} onChange={e=>setLimEdit({...limEdit,patronageSeenDays:Math.max(1,+e.target.value)})} style={{...INP,width:48}}/> <span style={{color:"#aaa"}}>j · Min</span> <input type="number" min={0} value={limEdit.patronageMinGeocoins??10} onChange={e=>setLimEdit({...limEdit,patronageMinGeocoins:Math.max(0,+e.target.value)})} style={{...INP,width:48}}/> <span style={{color:"#aaa"}}>geocoins</span></>, "Le bénéficiaire est cherché en cascade : d'abord parmi les joueurs connectés depuis moins de 24 h qui ne possèdent PAS le geocoin, puis moins de 7 jours, puis moins de N jours (ce réglage). Si tout le monde le possède déjà, le don part en doublon à un joueur connecté depuis moins de 24 h. Le critère choisi par le mécène ne fait que classer les candidats du palier retenu. Seuls les joueurs possédant au moins N geocoins sont éligibles."],
                  ["🎁 Mécénat — fenêtre de choix", <><input type="number" min={5} max={300} value={limEdit.patronageChoiceSeconds??20} onChange={e=>setLimEdit({...limEdit,patronageChoiceSeconds:Math.max(5,+e.target.value)})} style={{...INP,width:60}}/> <span style={{color:"#aaa"}}>s pour choisir le bénéficiaire</span></>, "Temps laissé au mécène pour choisir son bénéficiaire avant que le round ne se clôture. Passé ce délai (ou onglet fermé), un bénéficiaire aléatoire est attribué d'office — le geocoin n'est jamais perdu."],
                  ["🎁 Mécénat — critères proposés", (()=>{const ce=limEdit.patronageCriteriaEnabled||{};const set=(k,v)=>setLimEdit({...limEdit,patronageCriteriaEnabled:{...ce,[k]:v}});const opts=[['nouveau','Nouveau'],['ancien','Ancien'],['rapide','Rapide'],['fidele','Fidèle'],['petite_collection','Petite collec.'],['grande_collection','Grande collec.']];return <div style={{display:"flex",flexWrap:"wrap",gap:10}}>{opts.map(([k,lbl])=>(<label key={k} style={{display:"flex",alignItems:"center",gap:4,color:"#ddd",fontSize:11}}><input type="checkbox" checked={ce[k]!==false} onChange={e=>set(k,e.target.checked)} style={{width:13,height:13}}/> {lbl}</label>))}</div>})(), "Active/désactive chaque critère proposé au mécène pour choisir le bénéficiaire. Si tous sont décochés, le mécénat n'est plus proposé (le joueur au plafond joue pour la gloire)."],
                  ["🧪 Mécénat — bénéficiaire forcé (test)", <><input value={limEdit.patronageTestRecipient ?? ''} onChange={e=>setLimEdit({...limEdit,patronageTestRecipient:e.target.value})} placeholder="ex. qgx-admin — vide = normal" style={{...INP,width:200}}/></>, "MODE TEST : tant que ce pseudo est renseigné, TOUS les dons de mécénat vont à ce compte, quel que soit le critère choisi. Permet de tester le mécénat sans qu'un vrai joueur reçoive un cadeau. ⚠️ Vider ce champ avant la mise en production."],
                  ["🎁 Mécénat doublon — activer", <><label style={{display:"flex",alignItems:"center",gap:6,color:"#ddd",fontSize:11}}><input type="checkbox" checked={limEdit.patronageDuplicateEnabled!==false} onChange={e=>setLimEdit({...limEdit,patronageDuplicateEnabled:e.target.checked})} style={{width:14,height:14}}/> Autoriser l'offre d'un doublon depuis la collection</label></>, "Active le bouton 🎁 sur les doublons (collection + fiche) pour les offrir en mécénat. Décoché = fonctionnalité masquée."],
                  ["🎁 Mécénat doublon — communs / jour", <><input type="number" min={0} value={limEdit.patronageDailyCapCommun??2} onChange={e=>setLimEdit({...limEdit,patronageDailyCapCommun:Math.max(0,+e.target.value)})} style={{...INP,width:60}}/> <span style={{color:"#aaa"}}>communs / jour (0 = ∞)</span></>, "Nombre de doublons COMMUNS qu'un joueur peut offrir par jour. Les rares/épiques/légendaires ne sont offrables qu'au plafond hebdo d'acquisition atteint et consomment la limite hebdo de dons."],
                  ["🎁 Mécénat — points par rareté offerte", <><span style={{color:"#aaa"}}>Commun</span> <input type="number" min={0} value={limEdit.patronagePointsCommun??1} onChange={e=>setLimEdit({...limEdit,patronagePointsCommun:Math.max(0,+e.target.value)})} style={{...INP,width:50}}/> <span style={{color:"#aaa",marginLeft:6}}>Rare</span> <input type="number" min={0} value={limEdit.patronagePointsRare??2} onChange={e=>setLimEdit({...limEdit,patronagePointsRare:Math.max(0,+e.target.value)})} style={{...INP,width:50,marginLeft:4}}/> <span style={{color:"#aaa",marginLeft:6}}>Épique</span> <input type="number" min={0} value={limEdit.patronagePointsEpique??7} onChange={e=>setLimEdit({...limEdit,patronagePointsEpique:Math.max(0,+e.target.value)})} style={{...INP,width:50,marginLeft:4}}/> <span style={{color:"#aaa",marginLeft:6}}>Lég.</span> <input type="number" min={0} value={limEdit.patronagePointsLegendaire??20} onChange={e=>setLimEdit({...limEdit,patronagePointsLegendaire:Math.max(0,+e.target.value)})} style={{...INP,width:50,marginLeft:4}}/> <span style={{color:"#aaa"}}>pts</span></>, "Points ajoutés à l'achievement « Le mécène » selon la rareté offerte (don en jeu ET doublon). Le compteur est un cumul de ces points ; rappelé sur la fiche du geocoin « Mecenat »."],
                  ["✨ Forcer N prochains brillants", <><input type="number" min={0} max={999} value={limEdit.forceShinyCount??0} onChange={e=>setLimEdit({...limEdit,forceShinyCount:Math.max(0,+e.target.value)})} style={{...INP,width:60}}/> <span style={{color:"#aaa"}}>prochains geocoins (0 = off)</span></>, "TEST : force les N prochains geocoins PvP en brillant, puis se désactive tout seul (compteur décrémenté à chaque geocoin)."],
                  ...(onTestPatronage ? [["🎁 Tester l'animation mécénat", <><button onClick={onTestPatronage} style={{background:"linear-gradient(135deg,#f9ca24,#e17055)",border:"none",color:"#1e3045",fontWeight:900,fontSize:12,padding:"7px 16px",borderRadius:8,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>Lancer la roulette</button></>, "TEST : ouvre l'animation de mécénat en mode preview (roulette avec des joueurs factices, aucun don réel) — pour visualiser sans avoir à gagner un geocoin."]] : []),
                  ...(onTestPatronageGift ? [["🎁 Tester l'écran du bénéficiaire", <><button onClick={onTestPatronageGift} style={{background:"linear-gradient(135deg,#00b894,#0e9f6e)",border:"none",color:"#fff",fontWeight:900,fontSize:12,padding:"7px 16px",borderRadius:8,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>Afficher la popup</button></>, "TEST : affiche la popup « X vous a offert ce geocoin » telle que la voit le joueur qui reçoit le cadeau."]] : []),
                  ...
                  ["🗄️ Dépôt — emplacements permanents", (()=>{const p=Array.isArray(limEdit.holdSlotPrices)?limEdit.holdSlotPrices:[150,400];const set=(idx,v)=>{const next=[...p];next[idx]=Math.max(0,+v);setLimEdit({...limEdit,holdSlotPrices:next})};return <><span style={{color:"#aaa"}}>1ᵉʳ</span> <input type="number" min={0} value={p[0]??150} onChange={e=>set(0,e.target.value)} style={{...INP,width:70}}/> <span style={{color:"#aaa",marginLeft:6}}>2ᵉ</span> <input type="number" min={0} value={p[1]??400} onChange={e=>set(1,e.target.value)} style={{...INP,width:70,marginLeft:4}}/> <span style={{color:"#aaa"}}>Or</span></>})(), "Prix d'achat permanent des emplacements 1 et 2 du dépôt d'attente (aucun emplacement gratuit)"],
                  ["🔑 Dépôt — location d'un emplacement", <><input type="number" min={0} value={limEdit.holdRentPrice??80} onChange={e=>setLimEdit({...limEdit,holdRentPrice:Math.max(0,+e.target.value)})} style={{...INP,width:70}}/> <span style={{color:"#aaa"}}>Or / dépôt</span></>, "Prix de location de l'emplacement temporaire (proposé dès qu'aucun emplacement permanent n'est libre, vaut un seul dépôt)"],
                  ["🔄 Dépôt — remplacement", <><input type="number" min={0} value={limEdit.holdReplacePrice??50} onChange={e=>setLimEdit({...limEdit,holdReplacePrice:Math.max(0,+e.target.value)})} style={{...INP,width:70}}/> <span style={{color:"#aaa"}}>Or</span></>, "Prix pour remplacer un geocoin quand le dépôt est plein (l'ancien est détruit, +1 PF de consolation)"],
                  ["🧤 Poches — boost horaire", <><input type="number" min={0} value={limEdit.pocketBoostPrice??100} onChange={e=>setLimEdit({...limEdit,pocketBoostPrice:Math.max(0,+e.target.value)})} style={{...INP,width:70}}/> <span style={{color:"#aaa"}}>Or → +</span><input type="number" min={1} value={limEdit.pocketBoostCards??10} onChange={e=>setLimEdit({...limEdit,pocketBoostCards:Math.max(1,+e.target.value)})} style={{...INP,width:55,marginLeft:4}}/> <span style={{color:"#aaa"}}>geocoins/h</span></>, "Agrandit les poches jusqu'à minuit : +N geocoins par HEURE, cumulable (la limite quotidienne s'applique toujours)"],
                  ["🎒 Sac — emplacements permanents", (()=>{const p=Array.isArray(limEdit.bagSlotPrices)?limEdit.bagSlotPrices:[500,1000,2000,4000,6000];const set=(idx,v)=>{const next=[...p];next[idx]=Math.max(0,+v);setLimEdit({...limEdit,bagSlotPrices:next})};return <div style={{display:"flex",flexWrap:"wrap",alignItems:"center",gap:4}}>{["1ᵉʳ","2ᵉ","3ᵉ","4ᵉ","5ᵉ"].map((lbl,i)=>(<span key={i} style={{display:"inline-flex",alignItems:"center",gap:3}}><span style={{color:"#aaa"}}>{lbl}</span><input type="number" min={0} value={p[i]??''} onChange={e=>set(i,e.target.value)} style={{...INP,width:62}}/></span>))}<span style={{color:"#aaa"}}>Or</span></div>})(), "Prix des 5 emplacements de sac permanents : chacun ajoute +1 geocoin à la limite QUOTIDIENNE, pour toujours"],
                  ["✨ Sac brillant — emplacements (PF)", (()=>{const p=Array.isArray(limEdit.shinyBagSlotPrices)?limEdit.shinyBagSlotPrices:[200,400,600];const set=(idx,v)=>{const next=[...p];next[idx]=Math.max(0,+v);setLimEdit({...limEdit,shinyBagSlotPrices:next})};return <div style={{display:"flex",flexWrap:"wrap",alignItems:"center",gap:4}}>{["1ᵉʳ","2ᵉ","3ᵉ"].map((lbl,i)=>(<span key={i} style={{display:"inline-flex",alignItems:"center",gap:3}}><span style={{color:"#aaa"}}>{lbl}</span><input type="number" min={0} value={p[i]??''} onChange={e=>set(i,e.target.value)} style={{...INP,width:62}}/></span>))}<span style={{color:"#aaa"}}>PF</span></div>})(), "Prix (en points de forge) des 3 emplacements de sac brillant : chacun ajoute +1 shiny à la limite QUOTIDIENNE de brillants, pour toujours"],
                  ...
                  <div key={def.id} style={{background:isEditing?'#ffffff0a':'#ffffff05',border:`1px solid ${isDup?'#f9ca2466':def.hidden?'#e1705544':def.active?'#ffffff10':'#e74c3c33'}`,borderRadius:10,overflow:'hidden',transition:'background .15s'}}>
                    ...
                    {isEditing&&(
                      <div style={{padding:'10px 14px 14px',borderTop:'1px solid #ffffff10',background:'#ffffff06'}}>
                        ...
                        {/* Image du geocoin — appliquée à la carte de base ET à tous les paliers évolutifs */}
                        <div style={{padding:"8px 10px",marginBottom:6,background:"#6c5ce70f",borderRadius:8,border:"1px solid #6c5ce744",display:"flex",alignItems:"center",gap:12}}>
                          <div onClick={()=>{ if(def.card_id) achImgFileRef.current?.click(); else setMsg("❌ Lie d'abord un geocoin de base à cette condition."); }}
                            style={{width:44,height:57,borderRadius:7,border:"2px dashed #6c5ce777",background:(card?.image_url_thumb||card?.image_url)?"transparent":"#ffffff08",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,overflow:"hidden"}} title="Changer l'image">
                            {(card?.image_url_thumb||card?.image_url)?<img src={card.image_url_thumb||card.image_url} alt="" style={{width:"100%",height:"100%",objectFit:"contain"}}/>:<span style={{fontSize:18,opacity:.5}}>🖼️</span>}
                          </div>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontWeight:800,color:"#a29bfe",fontSize:10,marginBottom:2}}>🖼️ Image du geocoin</div>
                            <div style={{fontSize:9.5,color:"#8daacc",lineHeight:1.4}}>PNG carré, fond transparent. Appliquée {evo?"à tous les paliers (commun → légendaire)":"à la carte"} de cet achievement.</div>
                          </div>
                          <button onClick={()=>{ if(def.card_id) achImgFileRef.current?.click(); else setMsg("❌ Lie d'abord un geocoin de base à cette condition."); }}
                            style={{...BTN("#6c5ce722"),border:"1px solid #6c5ce744",color:"#a29bfe",padding:"5px 12px",borderRadius:7,fontSize:11,flexShrink:0}}>📁 Choisir un PNG</button>
                          <input ref={achImgFileRef} type="file" accept=".png,image/png" style={{display:"none"}}
                            onChange={e=>{
                              imgUpload(e, async({imageBase64})=>{
                                if(!imageBase64)return;
                                setMsg("⏳ Mise à jour de l'image…");
                                const {data,error}=await apiEditAchievementCard(def.card_id,{name:card?.name||def.name||'Achievement',description:card?.description||def.description||'',rarity:card?.rarity,image_url:imageBase64});
                                if(error){setMsg("❌ "+error);return;}
                                const finalImg=data?.card?.image_url||imageBase64;
                                const sibIds=[def.card_id,def.card_id_rare,def.card_id_epic,def.card_id_legendary].filter(Boolean).map(Number);
                                setAchCards(prev=>prev.map(c=>sibIds.includes(c.id)?{...c,image_url:finalImg,image_url_thumb:null}:c));
                                sibIds.forEach(id=>{const c=achCards.find(x=>x.id===id);if(c)onUpdateCardInPool?.({...c,image_url:finalImg,image_url_thumb:null});});
                                setMsg("✅ Image mise à jour"+(evo?" (tous les paliers)":"")+" !");
                              }, {name:card?.name||def.name||'',type:"Achievements",rarity:card?.rarity||'commun'});
                              e.target.value='';
                            }}/>
                        </div>
                        <div style={{display:"flex",gap:6}}>
