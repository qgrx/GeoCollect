...
export default function TresorPage({ dailyOffer, onClaim, onReveal, cardPool = [], shopPacksConfig = null, packsLoading = false, shopTestMode = false, isAdmin = false, dailyOfferGold = 5, onOpenCgv, hold = null, onClaimHold }) {
  ...
  return (
    ...
    <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          {card ? (
            <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ background: claimed ? '#ffffff18' : `linear-gradient(135deg,${c1},${c2})`, border: 'none', color: claimed ? theme.textMuted : '#1e3045', padding: '9px 15px', borderRadius: 10, fontFamily: "'Nunito',sans-serif", fontWeight: 900, fontSize: 13, cursor: claimed ? 'default' : 'pointer', flexShrink: 0, transition: 'all .3s', opacity: claiming ? 0.6 : 1 }}>
      ...
      {/* ── Packs ── */}
      {shopPacksConfig !== null && (packsLoading || visiblePacks.length > 0) && <div>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: '#000000bb', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
        ...
        <div style={{ background: theme.bgSurface, borderRadius: 20, padding: '22px 24px', width: 'min(96vw,460px)', border: `1px solid ${theme.border}`, boxShadow: '0 24px 60px #000c', fontFamily: "'Nunito',sans-serif" }}>
          ...
          <div style={{ background: theme.overlay, borderRadius: 12, padding: '14px 16px', marginBottom: 16, border: `1px solid ${theme.border}` }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, borderTop: `1px solid ${theme.border}`, paddingTop: 10 }}>
