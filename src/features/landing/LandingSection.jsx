...
export default function LandingSection({ onOpenAuth }) {
  ...
  return (
    <div ref={containerRef} style={{
      ...
      {/* ══ SLIDE 0 — GÉOCOIN DU JOUR (sans CTA) ═══════════════════════════════ */}
      <section style={{
        ...
        {/* Indicateur de défilement — bien visible */}
        <div style={{ position: 'absolute', bottom: 28, left: '50%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, animation: 'scrollBounce 2s ease-in-out infinite' }}>
          <span style={{ fontSize: 11, fontWeight: 800, color: '#e65100', letterSpacing: 2, textTransform: 'uppercase' }}>{t('landing_scroll')}</span>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg,#fff8f2,#fef2e4,#fdefd4)', transform: `translateY(${p1}px)`, willChange: 'transform' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 65% 55% at 50% 45%,#e6510014 0%,transparent 70%)', transform: `translateY(${p1 * .6}px)`, willChange: 'transform', animation: 'glow 4s ease-in-out infinite' }} />
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg,#e65100,#ff8f00,#f9ca24)', transform: `translateY(${p1}px)`, willChange: 'transform' }} />

        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '0 28px', maxWidth: 420 }}>
          <div style={{ fontSize: 52, marginBottom: 20, transform: `translateY(${p1 * -0.1}px)`, willChange: 'transform', filter: 'drop-shadow(0 4px 12px #e6510033)' }}>🪙</div>
          <div style={{ fontFamily: "'Fredoka One',sans-serif", fontSize: 30, color: '#2d1a00', marginBottom: 14, lineHeight: 1.25 }}>
            {t('landing_hero_title')}
          </div>
          <div style={{ fontSize: 15, color: '#666', lineHeight: 1.75, maxWidth: 320, marginBottom: 36 }}>
            {t('landing_hero_sub')}
          </div>
          <button onClick={onOpenAuth} style={{ background: 'linear-gradient(135deg,#e65100,#ff8f00)', border: 'none', color: '#fff', padding: '16px 42px', borderRadius: 16, fontFamily: "'Nunito',sans-serif", fontWeight: 900, fontSize: 17, cursor: 'pointer', boxShadow: '0 6px 28px #e6510044' }}>
            {t('landing_hero_cta')}
          </button>
        </div>
      </section>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg,#f0f7ff,#e8f3fd,#f5f0ff)', transform: `translateY(${p2}px)`, willChange: 'transform' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 50% 60% at 30% 60%,#f9ca2415 0%,transparent 70%)', transform: `translateY(${p2 * .5}px)`, willChange: 'transform' }} />
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg,#e65100,#f9ca24,#e17055)', transform: `translateY(${p2}px)`, willChange: 'transform' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '0 24px', width: '100%', maxWidth: 420 }}>
          ...
          <div style={{ background: '#fff', border: '1.5px solid #e8e0f0', borderRadius: 18, padding: '18px 20px', width: '100%', marginBottom: 24, boxShadow: '0 4px 20px #6a1b9a0d', transform: `translateY(${p2 * -0.05}px)`, willChange: 'transform' }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg,#f0fdf7,#e6faf4,#f0fff8)', transform: `translateY(${p3}px)`, willChange: 'transform' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 55% 50% at 70% 40%,#00b89418 0%,transparent 70%)', transform: `translateY(${p3 * .5}px)`, willChange: 'transform' }} />
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg,#00897b,#26a69a)', transform: `translateY(${p3}px)`, willChange: 'transform' }} />

        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '0 24px', width: '100%', maxWidth: 420 }}>
          <div style={{ transform: `translateY(${p3 * -0.1}px)`, willChange: 'transform', marginBottom: 10, pointerEvents: 'none' }}>
          ...
          <div style={{ display: 'flex', gap: 18, justifyContent: 'center', marginBottom: 26, transform: `translateY(${p3 * -0.08}px)`, willChange: 'transform' }}>
            {[
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg,#fffcf0,#fef8e1,#fff8f2)', transform: `translateY(${p4}px)`, willChange: 'transform' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 50% 55% at 50% 50%,#f9ca2418 0%,transparent 70%)', transform: `translateY(${p4 * .5}px)`, willChange: 'transform' }} />
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg,#f57f17,#fbc02d)', transform: `translateY(${p4}px)`, willChange: 'transform' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '0 24px', width: '100%', maxWidth: 380 }}>
          <div style={{ fontSize: 48, marginBottom: 10, transform: `translateY(${p4 * -0.1}px)`, willChange: 'transform' }}>🏆</div>
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 24, transform: `translateY(${p4 * -0.06}px)`, willChange: 'transform' }}>
